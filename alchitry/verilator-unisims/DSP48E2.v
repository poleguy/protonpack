...
// DSP_ALU wires
  ...
  reg [2:0] CARRYINSEL_mux = 3'b0;
  reg  [2:0] CARRYINSEL_reg = 3'b0;
  reg [8:0] OPMODE_mux = 9'b0;
...
// DSP_A_B_DATA wires
  ...
  reg [B_WIDTH-1:0] B1_DATA_out = {B_WIDTH{1'b0}};
...
// DSP_MULTIPLIER wires
  ...
  reg [M_WIDTH-2:0] ps_u_mask = 44'h55555555555;
  reg [M_WIDTH-2:0] ps_v_mask = 44'haaaaaaaaaaa;

// DSP_M_DATA wires
  reg [M_WIDTH-1:0] U_DATA_reg = {1'b1, {M_WIDTH-1{1'b0}}};
  reg [M_WIDTH-1:0] V_DATA_reg = {1'b1, {M_WIDTH-1{1'b0}}};
...
//*********************************************************
   ...
   always @(posedge CLK_in) begin
      if  (RSTM_in || (MREG_BIN == 1'b0) || glblGSR) begin
         U_DATA_reg <= {1'b1, {M_WIDTH-1{1'b0}}};
         V_DATA_reg <= {1'b1, {M_WIDTH-1{1'b0}}};
...
//*********************************************************
   ...
   always @(posedge CLK_in) begin
      if (RSTD_in || (DREG_BIN == 1'b0) || glblGSR) begin
   ...
   assign D_DATA    = (DREG_BIN == 1'b1) ? D_DATA_reg : D_in;
...
//*********************************************************
   ...
   assign AD_DATA    = (ADREG_BIN == 1'b1) ? AD_DATA_reg : AD_in;
