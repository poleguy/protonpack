/*******************************************************
assembler for ESM microcontroller
...
#define MAX_LINE_COUNT 5000 /* max 1000 lines allowed */
#define PROGRAM_COUNT 1024	/* total program word */
...
#define REG_COUNT 32			/* max 32 namereg can be declared */
...
struct opcode op[MAX_LINE_COUNT]; /* operaton array to save info for each line */
...
/*====================================== */
/* Only S00 - S1F are valid */
int register_number(char *s)
{
	if(*s != 'S') return( -1 );
	if(strlen(s) != 3) return( -1 );
	if(htoi(s+1)<=31)return(htoi(s+1));
	else return (-1);
}

/*====================================== */
/* Only (S00) - (S1F) are valid */
int register_indirect_number(char *s)
{
	static char temp_string[3];
	...
	if(htoi(temp_string)<=31)return(htoi(temp_string));
	else return (-1);
}
...
void insert_instruction(char *s, int p)
{
	...
	program_word[p] = program_word[p] | (n << 13);
}
...
{
	program_word[p] = program_word[p] | (unsigned) (c << 3);
}
...
{
	program_word[p] = program_word[p] | (unsigned) (c << 10);
}
...
{
	char *ptr;
    /* \r is needed to not error cryptically on dos style line endings */
    /* otherwise dos2unix was needed */
	char seps[]   = " :;,\t\r\n";
    char *token;
	...
	//printf("%s %s %s\n",op[line_count].instruction,op[line_count].op1,op[line_count].op2);
	return (0);
}
...
void test_instructions(void)
{
	...
	for(i = 0; i < line_count; i++){
        //printf("line %d\n",i); 
		//printf("%s %s %s\n",op[i].instruction,op[i].op1,op[i].op2);
		...
		if(op[i].instruction != NULL){
			...
			switch (j)
			{
				...
				case 20: /* RL */
					...
					else if(j == 13) reg_n = reg_n + SRX;
					else if(j == 18) reg_n = reg_n + SLX;
					else if((j == 14) || (j == 19)) reg_n = reg_n + SRA_SLA;
					else if(j == 15) reg_n = reg_n + ROR;
					else if(j == 20) reg_n = reg_n + ROL;
					insert_constant(reg_n, op[i].address);
					break;
				case 21: /* INPUT */
					if((reg_n = find_namereg(op[i].op1)) != -1)
					...
					if((reg_n = find_constant(op[i].op2)) != -1){
						...
						insert_constant(reg_n, op[i].address);
						insert_instruction(input_p_to_x_id,op[i].address);
					} else if((reg_n = register_indirect_number(op[i].op2)) != -1){
						insert_sYY(reg_n, op[i].address);
						insert_instruction(input_y_to_x_id,op[i].address);//check instruction
					} else {
						printf("ERROR - Invalid operand %s on line %d\n",op[i].op2, i+1);
						fprintf(ofp,"ERROR - Invalid operand %s on line %d\n",op[i].op2, i+1);
						error++;
				...
				case 22: /* OUTPUT */
					...
					if((reg_n = find_constant(op[i].op2)) != -1){
						...
						insert_sYY(reg_n, op[i].address);
						insert_instruction(output_y_to_x_id,op[i].address);//check instruction
					} else {
						printf("ERROR - Invalid operand %s on line %d\n",op[i].op2, i+1);
						fprintf(ofp,"ERROR - Invalid operand %s on line %d\n",op[i].op2, i+1);
						error++;
...
int main(int argc, char **argv)
{
	...
	ptr = strstr(filename, ".psm");
	if (ptr == NULL){
		printf("\nInvalid file type, use .psm extension\n");
		exit(1);
	}
	*ptr = '\0';
	strcat(filename,".log");
