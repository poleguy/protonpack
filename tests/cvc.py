...
from compile_file_generation import compile_file_generation
...
def run_rtl_sim_cvc(
    ...
    compile_and_run_cvc(compile_list, cocotb_lib_dir, command, frame)
    # must restore directory here, because it might (will?) get deleted by pytest , and break everything!
    os.chdir(starting_cwd)


def compile_and_run_cvc(compile_list, cocotb_lib_dir, command, frame):
    module = inspect.getmodule(frame[0])
    caller_filename = module.__file__
    caller_module = module.__name__
    ...
    if False:
        # this is the old os.system way of doing it.
        # the new way uses bash.bash()
        # the old way is left as an alternative in case we need to debug this stuff

        command = f"{command} {vpi} > tmp.txt"
        # command = f"{command} {vpi}"

        # compile AND run rtl simulation
        # print(command)
        cmd_rtn = os.system(command)
        if cmd_rtn != 0:
            if cmd_rtn != 256:
                # todo: this needs a comment explaining why 256 is okay
                raise Exception(f"Non 0, Non 256 return value: {cmd_rtn}")
        # cmd_rtn = 0
        # output = bash.bash(command)
        # sys.exit(cmd_rtn)
        ...
        with open("tmp.txt", "r") as f:
            ...
            # print(results)
        with open("tmp.txt", "r") as f:
            for stdout_line in f.readlines():
